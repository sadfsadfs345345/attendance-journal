...
fun App() {
    ...
    MaterialTheme(colorScheme = if (darkTheme) darkColors else lightColors) { Surface(Modifier.fillMaxSize()) { Row(Modifier.fillMaxSize()) {
        NavigationRail(containerColor = MaterialTheme.colorScheme.surfaceVariant) { Spacer(Modifier.height(18.dp)); Icon(Icons.Default.List, "Журнал", Modifier.padding(10.dp).size(34.dp), tint = MaterialTheme.colorScheme.primary); HorizontalDivider(Modifier.padding(12.dp)); listOf(Icons.Default.List to "Журнал", Icons.Default.BarChart to "Статистика", Icons.Default.Group to "Студенты", Icons.Default.List to "Заявки", Icons.Default.List to "Архив", Icons.Default.Settings to "Настройки").forEachIndexed { i, pair -> NavigationRailItem(selected = selectedTab == i, onClick = { selectedTab = i }, icon = { Icon(pair.first, pair.second) }, label = { Text(pair.second) }) } }
